...
public class BannerViewPager extends RelativeLayout implements ViewPager.OnPageChangeListener, View.OnClickListener {
    ...
    private void setMarks() {
        ...
        for (int i = 0; i < mPoints.length; i++) {
            ...
            tvPoints.setPadding(10, 2, 2, 2);
            tvPoints.setText("━");
            tvPoints.setTextColor(Color.WHITE);
            tvPoints.setTextSize(20);
            linPoints.addView(tvPoints);
            if (local == i) {
                tvPoints.setTextColor(mContext.getResources().getColor(R.color.customer_color));
            }
        }
        addView(linPoints);
    ...
    private void setMarkerPosition(int pos) {
        ...
        for (int i = 0; i < linPoints.getChildCount(); i++) {
            ...
            if (position % mPoints.length == i) {
                tv.setTextColor(mContext.getResources().getColor(R.color.customer_color));
            } else {
